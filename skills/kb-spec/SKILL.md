---
name: kb-spec
description: Bridge between GitHub Spec Kit and the kb-* knowledge base. Queries the KB for prior art before each Spec Kit phase (specify, plan, tasks, implement) and writes the results into the feature's research.md as clearly labeled sections. After the feature is finalized, delegates to kb-docs-sync to mirror specs/**/*.md and plans/**/*.md into the KB. Use when the user says "new spec", "run speckit with KB context", "sync the spec into KB", or invokes /kb-spec directly with a phase argument.
